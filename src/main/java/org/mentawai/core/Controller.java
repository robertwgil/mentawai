...
public class Controller extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        try {
            monitor.await(1L, TimeUnit.MILLISECONDS);
        } catch (InterruptedException e) {
            System.err.println("Container shouldn't never give this thread for another request before it had finished thread=" + Thread.currentThread().getId());
            try {
                monitor.await();
            } catch (InterruptedException ignore) {}
            
        } finally {
            monitor = new CountDownLatch(1); // reset for next
        }
